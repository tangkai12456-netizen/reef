// ============================================================
//  lib/reefbot.js
//  สมองของ ReefBot — เรียก Gemini พร้อม fallback ไป local knowledge base
//
//  แยกออกมาจาก server.js เพื่อให้ใช้ร่วมกันได้ทั้ง 2 ที่:
//    • server.js                    — รันในเครื่องตอน dev
//    • netlify/functions/chat.mjs   — รันบน Netlify ตอน production
//  ต่างกันแค่วิธีอ่าน GEMINI_API_KEY จึงรับ apiKey เป็นพารามิเตอร์แทนการอ่าน env เอง
//
//  เขียนเป็น CommonJS เพราะ server.js เดิมใช้ require() ส่วนฝั่ง Netlify (.mjs)
//  import ผ่าน interop ของ Node ได้ตามปกติ
// ============================================================
...
async function callGeminiOnce(model, chatHistory, apiKey) {
...
async function askGemini(chatHistory, apiKey) {
  ...
  for (const model of GEMINI_MODELS) {
    for (let attempt = 0; attempt < ATTEMPTS_PER_MODEL; attempt++) {
      try {
        return await callGeminiOnce(model, chatHistory, apiKey);
      } catch (err) {
        lastErr = err;
...
module.exports = { askGemini, findResponse, REEFBOT_DEFAULT_REPLY };
